...
static int fm34_chip_init(struct i2c_client *client)
{
	...
	rc = gpio_request_one(TEGRA_GPIO_PO3, GPIOF_INIT_HIGH, "fm34_reset");
	if (rc) {
		pr_err("DSP: gpio_request_one failed for input %d\n", TEGRA_GPIO_PO3);
	}
	gpio_set_value(TEGRA_GPIO_PO3, 1);

	/* config PWDN# pin, default HIGH. */
	rc = gpio_request_one(TEGRA_GPIO_PBB6, GPIOF_INIT_HIGH, "fm34_pwdn");
	if (rc) {
		pr_err("DSP: gpio_request_one failed for input %d\n", TEGRA_GPIO_PBB6);
	}
...
static void fm34_power_switch_init(void)
{
	...
	ret = gpio_request_one(dsp_1v8_power_control, GPIOF_INIT_HIGH, "dsp_power_1v8_en");
	if (ret < 0)
...
static int fm34_suspend(struct device *dev)
{
	...
	if(tegra3_get_project_id() == TEGRA3_PROJECT_TF201){
		...
		ret = gpio_request_one(TEGRA_GPIO_PA2, GPIOF_INIT_LOW, "dap2_fs");
		if (ret < 0)
		...
		ret = gpio_request_one(TEGRA_GPIO_PW4, GPIOF_INIT_LOW, "audio_mclk");
		if (ret < 0)
			pr_err("%s: gpio_request failed for gpio %s\n",
				__func__, "AUDIO_MCLK");
