...
static int __init cardhu_wifi_init(void)
{
	...
	rc = gpio_request_one(CARDHU_WLAN_PWR, GPIOF_INIT_LOW, "wlan_power");
	if (rc)
		pr_err("WLAN_PWR gpio request failed: %d\n", rc);

	rc = gpio_request_one(CARDHU_WLAN_RST, GPIOF_INIT_LOW, "wlan_rst");
	if (rc)
		pr_err("WLAN_RST gpio request failed: %d\n", rc);

	rc = gpio_request_one(CARDHU_WLAN_WOW, GPIOF_DIR_IN, "bcmsdh_sdmmc");
	if (rc)
